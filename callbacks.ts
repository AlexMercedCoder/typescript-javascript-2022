...
myArray1.forEach((item: number, index: number): void => {
  console.log(`Item: ${item} - Index: ${index}`);
});

// map - run a function on each item in the array and return a new array of the return values

console.log(myArray1.map((item: number, index: number): number => item + 1));


// filter - reuturn a new array of only items in which the function returns true
...
// some and every, return true or false based on whether the callback functions returns true some of the time or every time

console.log("some:", myArray1.some(isItEven))
console.log("every:", myArray1.every(isItEven))

// find and findIndex, return the first item in which the function returns true (find) or return the index of the first item that returns true

console.log("find:", myArray1.find(isItEven))
console.log("findIndex:", myArray1.findIndex(isItEven))

// reduce - takes an array of values and uses them to calculate one value
